...
namespace gem5
{
    ...
    {
        #ifdef TME
            extern unsigned char TME_tb[16][16];
            extern std::set<uint64_t> TMEAddrSet;
        #endif
        ...
        extern std::map<uint64_t, uint64_t> sim_to_host;
        ...
        {
            auto iter = sim_to_host.lower_bound(paddr);
            //--iter;
            assert(iter!=sim_to_host.end() && iter->first>=paddr && "simulated addr map to host table not exist");
            return (uint8_t *)(paddr - iter->first + iter->second);
        }
