...
{   
    ...
    {
        #ifdef TME
            unsigned char TME_tb[16][16]={0};
            std::set<uint64_t> TMEAddrSet;
        #endif
        bool hasInit=false;
        ...
        std::map<uint64_t, uint64_t> sim_to_host;
        ...
        {
            #ifdef  TME
                assert(0 && "illegal instruction in model TME");
                // return -1;
            #endif
        ...
        void setKeyID (uint64_t vaddr)
        {
            ...
            //assert(iterator == TMEAddrSet.end());//不支持中途修改密钥：安全性,防止加解密不一致
            uint64_t paddr=iteratorv->second;
            auto iterator=cbitMapping[mapInd].find(paddr);
        ...
        void enc_dec_init()
        {
            ...
            for(int i = 1;i <= 2;i++)
            {
                std_rand_get(sampleKeys[i].priKey,32,(unsigned char*)sm2::SM2_n);
                sm2::SM2_KeyGenToArray(sampleKeys[i].priKey,sampleKeys[i].PubKey.x,sampleKeys[i].PubKey.y);
            }
        ...
        int CreateIntKeySM4()
        {
            ...
            printf("create interal key:\n");
            for(int i=0;i<16;i++)
                printf("%x", (unsigned char)insideTab[index].internalKey[i]);
            printf("\n");
            return index;
        } 
        int CreateExtKeySM4(uint64_t lpwd,uint64_t hpwd)
        {
            #ifdef  TME
                assert(0 && "illegal instruction in model TME");
                // return -1;
            ...
            // printf("create shared key:\n");
            // for(int i=0;i<16;i++)
            //     printf("%02x", (unsigned char)insideTab[index].externalKey[i]);
            // printf("\n");
            return index;
        }
        ...
        { 
            // printf("in func enc_dec_engine (model=%d)\n",model);
            ...
            #ifdef TME
                if(!KeyCheck(TME_tb[pAddr%0xF]))
                {
                    std_rand_get(TME_tb[pAddr%0xF],16,sm4::SM4_n);
                    printf("TME: create key for%ld\n",pAddr%0xF);
                }
                if(alg==sm4_encry)//enc
                {   
                    sm4Encrypt(dataPtr,TME_tb[pAddr%0xF],true,pAddr);
                    ifdo=true;
                    TMEAddrSet.insert(pAddr);//这是解决该地址未加密就解密
                }
                else if(alg==sm4_decry)//dec
                {
                    std::set<uint64_t>::iterator it=TMEAddrSet.end();
                    if(TMEAddrSet.find(pAddr)!=TMEAddrSet.end())//这是解决该地址未加密就解密
                    {
                        assert(KeyCheck(TME_tb[pAddr%0xF]));
                        sm4Decrypt(dataPtr,TME_tb[pAddr%0xF],true,pAddr);
                        ifdo=true;
                    }
                }
                return ifdo;
            ...
            {
                pk = keyID?insideTab[index].externalKey:insideTab[index].internalKey;
                // printf("in func enc_dec:%d\n",keyID);
                assert(KeyCheck(pk));//需要判断是否存在
            }
            if(alg==sm4_encry)//enc
            {   
                sm4Encrypt(dataPtr,pk,true,pAddr);
            ...
            else if(alg==sm4_decry)//dec
            {
                ...
                //if(TMEAddrSet.find(pAddr)!=TMEAddrSet.end())//这是解决该地址未加密就解密
                // {
                sm4Decrypt(dataPtr,pk,true,pAddr);
                insideTab[index].freq++;
                ifdo=true;
                // }
            }
            ...
            // FILE *file=fopen("/home/yqy/桌面/stats/encTimes.txt","a+");
            // fprintf(file,"enc times:%ld  dectimes:%ld\n",countForEnc,countForDec);
            // fclose(file);
            return ifdo;
        }

        const char netWorkSimFile[] = "/home/yqy/桌面/testCode/simNetWork/simlog.txt";
        const char AttestationSimFile[] = "/home/yqy/桌面/testCode/simNetWork/simAttestation.txt";  
        const char memTransDataSimFile[] = "/home/yqy/桌面/testCode/simNetWork/memTransData.txt";
